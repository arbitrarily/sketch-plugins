...
function ___REQUIRE___(identifier) { // eslint-disable-line no-unused-vars
  ...
  const importURL = NSURL.fileURLWithPath(this.scriptURL)
  ...
  if (alreadyLoadedModule) {
    ...
    if (importedData === null) {
      throw `Unable to import file '${importURL.path()}' (from '${this.scriptURL}'). Does the file exist?`;
    }
    ...
    const modulePreambleCode = '(function(){' +
      ...
      'const require=___REQUIRE___.bind({scriptURL:"' + importURL.path().replace('"', '\\"') + '"});' +
      '/* MODULE BEGIN */';
...
const require = ___REQUIRE___.bind({ // eslint-disable-line no-unused-vars
  scriptURL: COScript.currentCOScript().env().objectForKey('scriptURL').path()
});
